...
export async function run({ interaction, profileOf }: CommandData) {
	...
	try {
		...
		await interaction.reply({
			content:
				'Sorry, Minco Penguin failed to fetch the members of this server. Please try using the command again!',
			ephemeral: true
		});
		return;
	}
	...
	const authorIndex = formatted.findIndex(
	...
	const lbEmbed = new MessageEmbed()
		...
		.setFooter(
			`Page 1/${slices.length} • Your leaderboard rank: ${
				authorIndex + 1
			}`
		);
		...
		if (i.customId === 'prev') {
			...
			lbEmbed
				...
				.setFooter(
					`Page ${currentPage + 1}/${
						slices.length
					} • Your leaderboard rank: ${authorIndex + 1}`
				);
