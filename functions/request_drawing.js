...
module.exports =
    ...
    async (message, args, client, profileData) => {
        ...
        const collector = reactMsgCl.createReactionCollector(filter, { time: ms('30m') });
        ...
        collector.on('collect', async (reaction, user) => {
            ...
            if (reaction.emoji.name == "🚫") {
                ...
                if (!accepted) claire.send("Accept the request first before choosing a price.").then(msg => {
                    setTimeout(() => {
                        msg.delete();
                    }, 5000);
                });
                    ...
                    if (isNaN(price)) claire.send("Enter a valid number");
                        ...
                        if (price > profileData.mincoDollars) {
                            message.author.send(`Claire has decided on a price of ${price} dollars. However, you don't have that much money. (you can send Claire a message by using the :telephone_receiver: button)`);
                        ...
                        confirmCollector.on('collect', async (reaction, user) => {
                            if (reaction.emoji.name == '✅') {
                                ...
                                await profileModel.findOneAndUpdate({ userID: '802668636795830292' }, {
                                    $inc: {
                                        mincoDollars: price
                                    }
                                });
                                claire.send(`${message.author.toString()} has confirmed the price. You have received ${price} Minco Dollars`);
                                message.author.send(`You have accepted the price. ${price} Minco Dollars from your profile have been given to Claire.`);
                            }
                            else {
