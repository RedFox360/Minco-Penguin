...
const blackjackCommand = new SlashCommand()
	...
	.setRun(async interaction => {
		...
		const playerHasBlackjack = blackjack.isBlackjack(
			playerHands[0]
		);
		...
		const playerValues = [
			blackjack.getValue(playerHands[0], true)
		];
		if (blackjackOutcome) {
			await interaction.reply({
				embeds: [
					blackjack.getEmbed(
						...
						blackjackOutcome,
						blackjack.calculateOutcomeData(
							blackjackOutcome,
							bets
						)
					)
		...
		const split = new MessageButton()
			...
			.setStyle('SECONDARY')
			.setDisabled(
				playerHands[0][0].display !==
					playerHands[0][1].display
			);
		...
		const pushOutcome = (dealerValue: number) => {
			if (playerValues[currentHand].value > 21) {
				...
				dealerValue < playerValues[currentHand].value
			) {
