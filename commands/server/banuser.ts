...
export const name = "banuser";
export const description = "[ADMIN ONLY] Ban a user from using Minco Penguin\nUnban: unban a user";
export const aliases = ["unban"];
export const usage = "!banuser <@user>";
export async function execute(message, _, cmd) {
	if (message.author.id == "724786310711214118") {
		let guildData: any = serverModel.findOne({ serverID: message.guild.id });
