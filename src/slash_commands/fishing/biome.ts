...
const biome = new SlashCommand()
	...
	.setRun(async interaction => {
		...
		if (profile.fish.xp >= 1000)
			unlockedOptionsValues.push('river');
		if (profile.fish.xp >= 2000)
			unlockedOptionsValues.push('pond');
		if (profile.fish.xp >= 3000)
			unlockedOptionsValues.push('warm ocean');
		if (profile.fish.xp >= 4000)
			unlockedOptionsValues.push('beach');
			...
			.setDescription(
				biomeMenu
					.map(a =>
						formatBiome(
							a,
							!unlockedOptionsValues.includes(a.value)
						)
					)
					.join('\n')
			)
