...
const weather = new SlashCommand()
	.setCommandData(builder =>
		builder
			.setName('weather')
			.setDescription(
				'Get the weather from a location (from MSN)'
			)
			...
			.addStringOption(option =>
				option
					.setName('degree_type')
					.setDescription(
						'The degree type for the weather forecast'
					)
					.setRequired(false)
					.addChoice('Celsius', 'C')
...
async function run(
	...
	if (interaction.isCommand()) {
		...
		const degreeOption =
			interaction.options.getString('degree_type');
...
*Low*: ${dailyForecast.low}°${degreeType}`;
		...
		if (
			dailyForecast.precip !== '' &&
			dailyForecast.precip !== '0'
		) {
	...
	const collector = msg.createMessageComponentCollector({
		filter: i =>
			i.customId === 'view_weather' ||
			i.customId === 'view_forecast',
		time: collectorTime,
